...
| 版本 | V1.0-r5（双运行时 + 十万级检索 + 工具层/会话层/运维层落地回写） |
| 日期 | 2026-09-13 |
| 状态 | 设计稿，待评审；**M0 依赖 spike 已完成**（2026-09-12，见 `docs/M0-依赖spike-实测报告.md`）；**M1~M4 已实现并端到端验收**（见 `docs/M1~M4-实现报告.md`）；**遗留：19.1 真实 LLM/embedding 选型 + 评测集规模补齐**（见 M4 报告 §6） |
...
| **M4 可观测 + 治理 + 评测**（**已完成 2026-09-13**） | `/metrics`（Prometheus 文本）+ `/readyz`（依赖逐项布尔）+ 管理端（ADMIN_TOKEN 鉴权 + 全量查询 + 工单状态机写接口 + `admin.html` 大盘）+ trace 全链路（`X-Trace-Id` + 结构化日志）+ AI 链路双闸限流 + **密钥治理**（`app.env` 出库 + `.example` + 泄露面自查）+ 三套评测集 + 诚实性验收机检 + 降级演练（停编排层/停 Redis/无 embedding key）+ compose 补 `qdrant`/`orchestrator` | 大盘与日志口径一致；三档评测出数；**入口真实性 0 违规**；**停编排层后 `/cs/ask` 仍 200 且有工单号**；`git add --dry-run app.env` 被忽略 —— 实测见 `docs/M4-实现报告.md`（**边界声明覆盖率与三套评测规模未达门槛，已如实标注并进滞留清单**） |
